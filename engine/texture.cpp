...
void create_texture_image(Vulkan_Context& vulkan_context, Command_Buffer_Context& command_buffer_context)
{
    ...
    create_buffer(vulkan_context, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
    ...
    create_image(vulkan_context, command_buffer_context, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB,
                 VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    transition_image_layout(vulkan_context, command_buffer_context, command_buffer_context.texture_image,
                            VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(vulkan_context, command_buffer_context, stagingBuffer, command_buffer_context.texture_image,
...
void create_image(Vulkan_Context& vulkan_context, Command_Buffer_Context& command_buffer_context, uint32_t width,
    uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties)
{
    ...
    if (vkCreateImage(vulkan_context.logical_device, &imageInfo, nullptr, &command_buffer_context.texture_image) !=
        VK_SUCCESS)
    {
    ...
    vkGetImageMemoryRequirements(vulkan_context.logical_device, command_buffer_context.texture_image, &memRequirements);
    ...
    if (vkAllocateMemory(vulkan_context.logical_device, &allocInfo, nullptr,
                         &command_buffer_context.texture_image_memory) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to allocate image memory!");
    }

    vkBindImageMemory(vulkan_context.logical_device, command_buffer_context.texture_image,
                      command_buffer_context.texture_image_memory, 0);
}
...
{
    command_buffer_context.texture_image_view = create_image_view(vulkan_context, command_buffer_context.texture_image,
...
void create_texture_sampler(Vulkan_Context& vulkan_context, Command_Buffer_Context& command_buffer_context)
{
    ...
    *VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER: Return a solid color when sampling beyond the dimensions of the image.
    ...
    if (vkCreateSampler(vulkan_context.logical_device, &samplerInfo, nullptr, &command_buffer_context.texture_sampler)
        != VK_SUCCESS)
    {
