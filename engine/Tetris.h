...
inline void spawn_block(Tetromino& Tetromino, glm::vec3 color, VERTEX_DYNAMIC_INFO& vertex_info)
{
    ...
    for (auto grid_position : Tetromino.tetromino_default_position)
    {
        ...
        Tetromino.id.emplace_back(add_quad(glm::vec2{XOFFSET + (CELL_SIZE*grid_position.x), YOFFSET + (CELL_SIZE*grid_position.y)}, color, BLOCK_SCALE, vertex_info));
    }
}
